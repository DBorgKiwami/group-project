...
var tutorial_lines: Array[Dictionary] = [
	{"key": "TAB", "text": "GRAPPLE TO THE HIGHLIGHTED POINTS", "input": "grapple"},
	{"key": "TAB", "text": "YOU CAN CHAIN GRAPPLES IF YOU ARE CLOSE ENOUGH!", "input": "grapple"},
]
...
func _draw() -> void:
	...
	var box_size := Vector2(650, 80)

	var box_pos := Vector2(
		(viewport_size.x - box_size.x) / 2.0,
		viewport_size.y - 140.0
	)
	...
	draw_key_box(
		box_pos + Vector2(14, 20),
		key
	)

	draw_pixel_text(
		box_pos + Vector2(125, 31),
		shown_text,
		2,
		WHITE
	)
