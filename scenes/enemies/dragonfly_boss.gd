...
func _physics_process(delta: float) -> void:
	if _dying:
		if _death_falling:
			...
			# A completed attack tween must never move the corpse again.
			velocity = Vector3.ZERO
		return
	move_and_slide()
...
func _play_idle_visual(request_id: int) -> void:
	...
	# The longer idle_prepare silhouette stays at flight height. Switch to the
	# actual perched pose before descending so the preparation frame cannot
	# penetrate the floor at the lowest point.
	sprite.play(&"idle")
	...
	_perch_tween.tween_property(sprite, "position", perch_target, 0.25)
...
func _wake_from_perch() -> void:
	...
	_perch_tween = create_tween()
	_perch_tween.set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	_perch_tween.tween_property(sprite, "position", _sprite_anchor, 0.12)
...
func _find_visual_perch_target():
	...
	# Sprite3D maps the top side of this texture toward world-down in this
	# scene. idle2's visible silhouette reaches 1.20 units below its centre.
	var desired_center_y := (
		float(surface_y)
		+ idle_sprite_bottom_extent
		+ perch_ground_clearance
	)
	var local_drop := desired_center_y - global_position.y - _sprite_anchor.y
	local_drop = clampf(local_drop, -perch_visual_drop_limit, 0.0)
...
func _start_death_fall() -> void:
	...
	# Always recoil toward the arena centre. Moving away from the player could
	# push a boss defeated near either edge completely outside the camera.
	...
	var surface_y = _find_ground_y_at(landing_probe, 30.0)
	_death_landing_y = start_position.y - 6.0
	if surface_y != null:
		# The final death sprite reaches 0.69 units toward world-down from its
		# centre. Place that visible edge directly on the main floor.
		_death_landing_y = (
			float(surface_y)
	...
	_death_velocity = Vector3(
		(_death_landing_x - start_position.x) / maxf(death_fall_duration, 0.1),
...
func _update_death_fall(delta: float) -> void:
	# Death uses its own velocity so an attack tween cannot overwrite gravity.
	velocity = Vector3.ZERO
	_death_velocity.y -= death_gravity * delta
	global_position += _death_velocity * delta
	if (
		(_death_velocity.x < 0.0 and global_position.x <= _death_landing_x)
		or (_death_velocity.x > 0.0 and global_position.x >= _death_landing_x)
	):
		global_position.x = _death_landing_x
		_death_velocity.x = 0.0
	if global_position.y <= _death_landing_y and _death_velocity.y <= 0.0:
		global_position.x = _death_landing_x
...
func _find_surface_y(max_distance: float):
	return _find_surface_y_at(global_position, max_distance)


func _find_ground_y_at(world_position: Vector3, max_distance: float):
	if is_instance_valid(ground_reference):
		return ground_reference.global_position.y
	return _find_surface_y_at(world_position, max_distance)


func _find_surface_y_at(world_position: Vector3, max_distance: float):
...
func _is_player_near() -> bool:
	...
	return is_instance_valid(player) and global_position.distance_to(player.global_position) <= player_awareness_distance
...
func _visual_request_is_current(request_id: int, expected_state: String) -> bool:
	return not _dying and request_id == _visual_generation and _last_state_name == expected_state


func _stop_perch_tween() -> void:
	if is_instance_valid(_perch_tween) and _perch_tween.is_valid():
		_perch_tween.kill()
