...
var tutorial_lines: Array[Dictionary] = [
	{"key": "WASD", "text": "MOVE", "input": ""},
	{"key": "SPACE", "text": "JUMP", "input": "ui_accept"},
	{"key": "SHIFT", "text": "SPRINT", "input": "sprint"},
]
...
func _input(event: InputEvent) -> void:
	...
	# First tutorial screen: press Space to continue.
	if current_index == 0:
		if event.is_action_pressed("ui_accept"):
			_next_tutorial()
		return

	var current_input: String = str(
		tutorial_lines[current_index]["input"]
	)

	if current_input != "" and event.is_action_pressed(current_input):
		_next_tutorial()
...
func _next_tutorial() -> void:
	...
	# First press finishes typing the current instruction.
	if visible_characters < current_text.length():
