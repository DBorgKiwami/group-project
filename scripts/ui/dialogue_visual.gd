...
func draw_dialogue_screen() -> void:
	...
	var panel_pos := Vector2(size.x / 2.0 - 250, size.y - 180)

	draw_rect(Rect2(Vector2.ZERO, size), Color(0.010, 0.014, 0.015, 0.24))
	draw_name_plate(panel_pos + Vector2(10, -20), Vector2(150, 30))
	draw_panel(panel_pos, Vector2(500, 150))
	draw_portrait_box(panel_pos + Vector2(12, 24))
	draw_dialogue_text(panel_pos + Vector2(150, 40))
	draw_continue_hint(panel_pos + Vector2(470, 120))
...
func draw_name_plate(pos: Vector2, size: Vector2) -> void:
	...
	draw_pixel_text_center(pos + Vector2(size.x / 2, 9), speaker_name, 2, CREAM)
...
func draw_portrait_box(pos: Vector2) -> void:
	draw_rect(Rect2(pos, Vector2(110, 100)), SHADOW)
	draw_rect(Rect2(pos + Vector2(4, 4), Vector2(102, 92)), FRAME)
	draw_rect(Rect2(pos + Vector2(8, 8), Vector2(94, 84)), FRAME_LIGHT)
	draw_rect(Rect2(pos + Vector2(14, 10), Vector2(80, 80)), PORTRAIT_BG)
	draw_texture_rect(PLAYER_PORTRAIT, Rect2(pos + Vector2(16, 10), Vector2(74, 80)), false)
...
func draw_dialogue_text(pos: Vector2) -> void:
	...
	var shown_count: int = clampi(visible_letters, 0, current_line.length())
	var shown: String = current_line.substr(0, shown_count)

	var wrapped: Array = _wrap_text(shown, 40)

	for i in range(wrapped.size()):
		draw_pixel_text(pos + Vector2(0, i * 22), wrapped[i], 2, WHITE)


func _wrap_text(text: String, max_chars: int) -> Array:
	var words := text.split(" ")
	var lines := []
	var current_line_buf := ""
	for word in words:
		if (current_line_buf + " " + word).length() > max_chars and current_line_buf != "":
