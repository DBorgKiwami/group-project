...
func draw_dialogue_text(pos: Vector2) -> void:
	if playingDialogue.is_empty():
		return
	var line: String = playingDialogue[dialogueIndex]
	var visible_letters := int(time * 30.0)
	var shown_count := clamp(visible_letters, 0, line.length())
	var shown := line.substr(0, shown_count)

	var wrapped := _wrap_text(shown, 40)
...
func _wrap_text(text: String, max_chars: int) -> Array:
	...
	var current_line := ""
	for word in words:
		if (current_line + " " + word).length() > max_chars and current_line != "":
