...
func _ready() -> void:
	...
	_start_waterfall_loop(left_waterfall)
	_start_waterfall_loop(right_waterfall)
	_vary_pond_layer()
	_play_breezes()
	_play_single_insect()
	_play_insect_group()
...
func _start_waterfall_loop(player: AudioStreamPlayer3D) -> void:
	if player == null or player.stream == null:
		return

	# Skip the quiet lead-in each time the short waterfall recording loops.
	var looping_stream := player.stream.duplicate()
	if looping_stream is AudioStreamOggVorbis:
		looping_stream.loop = true
		looping_stream.loop_offset = 0.5
	player.stream = looping_stream
	player.play()
